...
TEST_BEGIN
    ...
    c.beginFunction("main"); {
      ...
      c.beginBlock("entry"); {
        ...
        c.writeOut(c.add(c.eq("a", "b"), values::i8('A')));
        c.writeOut(c.add(c.eq("b", "a"), values::i8('A')));
        ...
        c.writeOut(c.add(c.eq("x", "y"), values::i8('A')));
        c.writeOut(c.add(c.eq("y", "x"), values::i8('A')));

        // mixed i8/i16 variables: 20 ? 2000, then 2000 ? 20
        c.writeOut(c.add(c.eq("a", "x"), values::i8('A')));
        c.writeOut(c.add(c.eq("x", "a"), values::i8('A')));

        // variable/literal equality edge: 20 ? 20
        c.writeOut(c.add(c.eq("a", values::i8(20)), values::i8('A')));

        // literal/variable with larger i16 literal: 4096 ? 3000
        c.writeOut(c.add(c.eq(values::i16(4096), "y"), values::i8('A')));

        // literal/literal constant folding, mixed widths: 42 ? 42
        c.writeOut(c.add(c.eq(values::i8(42), values::i16(42)), values::i8('A')));
