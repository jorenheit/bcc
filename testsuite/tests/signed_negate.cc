...
c.function("main").begin(); {
  ...
  c.block("entry").begin(); {
    ...
    c.writeOut(c.add(c.negate("a"), literal::s8('@'))); // A

    // 2. negate(s8 literal): -(2) = -2; -2 + 'D' = 'B'
    c.writeOut(c.add(c.negate(literal::s8(2)), literal::s8('D'))); // B

    // 3. negate(s8 negative literal): -(-3) = 3; 3 + '@' = 'C'
    c.writeOut(c.add(c.negate(literal::s8(-3)), literal::s8('@'))); // C

    // 4. negate(s16 slot): -(300) = -300
    // CAT('p', 'F') - 300 = CAT('D', 'E') -> DE
    c.assign("b", literal::s16(300));
    c.writeOut(c.add(c.negate("b"), literal::s16(CAT('p', 'F')))); // DE

    // 5. negate(s16 literal): -(-1) = 1
    // CAT('E', 'F') + 1 = CAT('F', 'F') -> FF
    c.writeOut(c.add(c.negate(literal::s16(-1)), literal::s16(CAT('E', 'F')))); // FF
