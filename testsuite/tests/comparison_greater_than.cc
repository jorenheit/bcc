...
TEST_BEGIN
    ...
    c.function("main").begin(); {
      ...
      c.block("entry").begin(); {
        ...
        c.writeOut(c.add(c.gt("a", "b"), literal::i8('A'))); // A
        c.writeOut(c.add(c.gt("b", "a"), literal::i8('A'))); // B
        ...
        c.writeOut(c.add(c.gt("a", "x"), literal::i8('A'))); // A
        c.writeOut(c.add(c.gt("x", "a"), literal::i8('A'))); // B

        // variable/literal equality edge: 20 ? 20
        c.writeOut(c.add(c.gt("a", literal::i8(20)), literal::i8('A'))); // A

        // literal/variable with larger i16 literal: 4096 ? 3000
        c.writeOut(c.add(c.gt(literal::i16(4096), "y"), literal::i8('A'))); // B

        // literal/literal constant folding, mixed widths: 42 ? 42
        c.writeOut(c.add(c.gt(literal::i8(42), literal::i16(42)), literal::i8('A'))); // A

        // assign variant, i8/i8 variables: 7 ? 9
        c.assign("a", literal::i8(7));
        c.assign("b", literal::i8(9));
        c.gtAssign("a", "b");
        c.writeOut(c.add("a", literal::i8('A')));  // A

        // assign variant, i16 variable/literal: 5000 ? 4000
        c.assign("x", literal::i16(5000));
        c.gtAssign("x", literal::i16(4000));
        c.writeOut(c.add("x", literal::i16(CAT('A', 'B')))); // BB
