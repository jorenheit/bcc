...
c.beginFunction("main"); {
  ...
  c.beginBlock("entry"); {
    ...
    c.writeOut(c.add(c.mul("x", "y"), values::i16(0x4141)));

    // 1 * 1 -> 1, mapped to "BB"
    c.assign("x", values::i16(1));
    c.assign("y", values::i16(1));
    c.writeOut(c.add(c.mul("x", "y"), values::i16(0x4241)));

    // x *= 0 -> 0, mapped to "CC"
    c.assign("x", values::i16(0x1234));
    c.assign("y", values::i16(0));
    c.mulAssign("x", "y");
    c.writeOut(c.add("x", values::i16(0x4343)));

    // 1 *= 1 -> 1, mapped to "DD"
    c.assign("x", values::i16(1));
    c.assign("y", values::i16(1));
    c.mulAssign("x", "y");
    c.writeOut(c.add("x", values::i16(0x4443)));
