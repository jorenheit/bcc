...
// Test logical OR (lor / lorAssign) on u8 args, u16 args, mixed args,
...
c.function("main").begin(); {
  ...
  // 1. u8 variable + u8 variable: true OR true -> 1
  c.assign("a", literal::u8(0x7B));
  c.assign("b", literal::u8(0x2D));
  c.write(c.add(c.lor("a", "b"), literal::u8('A')));

  // 2. u8 variable + u8 variable: true OR false -> 1
  c.assign("a", literal::u8(0x7B));
  c.assign("b", literal::u8(0));
  c.write(c.add(c.lor("a", "b"), literal::u8('A')));

  // 3. u16 variable + u16 variable: true OR true -> 1
  c.assign("x", literal::u16(0x1234));
  c.assign("y", literal::u16(0xBEEF));
  c.write(c.add(c.lor("x", "y"), literal::u8('A')));

  // 4. u16 variable + u16 variable: false OR true -> 1
  c.assign("x", literal::u16(0));
  c.assign("y", literal::u16(0xCAFE));
  c.write(c.add(c.lor("x", "y"), literal::u8('A')));

  // 5. mixed u8/u16 variables: true OR true -> 1
  c.assign("a", literal::u8(0x55));
  c.assign("x", literal::u16(0x4001));
  c.write(c.add(c.lor("a", "x"), literal::u8('A')));

  // 6. variable + literal: true OR false -> 1
  c.assign("a", literal::u8(0x66));
  c.write(c.add(c.lor("a", literal::u8(0)), literal::u8('A')));

  // 7. literal + variable: true OR true -> 1
  c.assign("x", literal::u16(0x2345));
  c.write(c.add(c.lor(literal::u16(0x7777), "x"), literal::u8('A')));

  // 8. literal + literal, constant-folded: true OR false -> 1
  c.write(c.add(c.lor(literal::u16(0x9999), literal::u16(0)), literal::u8('A')));

  // 9. u8 assign variant: true OR true -> 1
  c.assign("a", literal::u8(0x42));
  c.lorAssign("a", literal::u8(0x24));
  c.write(c.add("a", literal::u8('A')));

  // 10. u16 assign variant: false OR false -> 0
  // Use land(result, 1) to print the boolean value as a single output byte.
  c.assign("x", literal::u16(0));
  c.lorAssign("x", literal::u16(0));
