...
c.function("main").begin(); {
  ...
  c.block("entry").begin(); {
    ...
    c.writeOut(c.add("z", literal::s8('@'))); // A
    ...
    c.negateAssign("x");
    c.writeOut(c.add("x", literal::s8('@'))); // C

    // negateAssign(4) = -4; -4 + 'H' = 'D'
    c.assign("x", literal::s8(4));
    c.negateAssign("x");
    c.writeOut(c.add("x", literal::s8('H'))); // D

    // abs(-5) = 5; 5 + '@' = 'E'
    c.assign("x", literal::s8(-5));
    c.assign("z", c.abs("x"));
    c.writeOut(c.add("z", literal::s8('@'))); // E

    // abs(6) = 6; 6 + '@' = 'F'
    c.assign("x", literal::s8(6));
    c.assign("z", c.abs("x"));
    c.writeOut(c.add("z", literal::s8('@'))); // F

    // absAssign(-7) = 7; 7 + '@' = 'G'
    c.assign("x", literal::s8(-7));
    c.absAssign("x");
    c.writeOut(c.add("x", literal::s8('@'))); // G

    // absAssign(8) = 8; 8 + '@' = 'H'
    c.assign("x", literal::s8(8));
    c.absAssign("x");
    c.writeOut(c.add("x", literal::s8('@'))); // H
    ...
    c.signBitAssign("x");
    c.writeOut(c.add("x", literal::s8('J'))); // K

    // signBitAssign(127): x becomes 0; 0 + 'L' = 'L'
    c.assign("x", literal::s8(127));
    c.signBitAssign("x");
    c.writeOut(c.add("x", literal::s8('L'))); // L
