...
// Test logical NAND (lnand / lnandAssign) on i8 args, i16 args, mixed args,
...
c.beginFunction("main"); {
  ...
  c.beginBlock("entry"); {
    // 1. i8 variable + i8 variable: true NAND true -> 0
    c.assign("a", literal::i8(0x7B));
    c.assign("b", literal::i8(0x2D));
    c.writeOut(c.add(c.lnand("a", "b"), literal::i8('A')));

    // 2. i8 variable + i8 variable: true NAND false -> 1
    c.assign("a", literal::i8(0x7B));
    c.assign("b", literal::i8(0));
    c.writeOut(c.add(c.lnand("a", "b"), literal::i8('A')));

    // 3. i16 variable + i16 variable: true NAND true -> 0
    c.assign("x", literal::i16(0x1234));
    c.assign("y", literal::i16(0xBEEF));
    c.writeOut(c.add(c.lnand("x", "y"), literal::i8('A')));

    // 4. i16 variable + i16 variable: false NAND true -> 1
    c.assign("x", literal::i16(0));
    c.assign("y", literal::i16(0xCAFE));
    c.writeOut(c.add(c.lnand("x", "y"), literal::i8('A')));

    // 5. mixed i8/i16 variables: true NAND true -> 0
    c.assign("a", literal::i8(0x55));
    c.assign("x", literal::i16(0x4001));
    c.writeOut(c.add(c.lnand("a", "x"), literal::i8('A')));

    // 6. variable + literal: true NAND false -> 1
    c.assign("a", literal::i8(0x66));
    c.writeOut(c.add(c.lnand("a", literal::i8(0)), literal::i8('A')));

    // 7. literal + variable: true NAND true -> 0
    c.assign("x", literal::i16(0x2345));
    c.writeOut(c.add(c.lnand(literal::i16(0x7777), "x"), literal::i8('A')));

    // 8. literal + literal, constant-folded: true NAND false -> 1
    c.writeOut(c.add(c.lnand(literal::i16(0x9999), literal::i16(0)), literal::i8('A')));

    // 9. i8 assign variant: true NAND true -> 0
    c.assign("a", literal::i8(0x42));
    c.lnandAssign("a", literal::i8(0x24));
    c.writeOut(c.add("a", literal::i8('A')));

    // 10. i16 assign variant: false NAND false -> 1
    // Use land(result, 1) to print the boolean value as a single output byte.
    c.assign("x", literal::i16(0));
    c.lnandAssign("x", literal::i16(0));
