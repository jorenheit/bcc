// Greater-than-or-equal comparison tests (ge / geAssign)
...
TEST_BEGIN
    ...
    c.beginFunction("main"); {
      ...
      c.beginBlock("entry"); {
        ...
        c.writeOut(c.add(c.ge("a", "b"), literal::i8('A')));
        c.writeOut(c.add(c.ge("b", "a"), literal::i8('A')));
        ...
        c.writeOut(c.add(c.ge("x", "y"), literal::i8('A')));
        c.writeOut(c.add(c.ge("y", "x"), literal::i8('A')));

        // mixed i8/i16 variables: 20 ? 2000, then 2000 ? 20
        c.writeOut(c.add(c.ge("a", "x"), literal::i8('A')));
        c.writeOut(c.add(c.ge("x", "a"), literal::i8('A')));

        // variable/literal equality edge: 20 ? 20
        c.writeOut(c.add(c.ge("a", literal::i8(20)), literal::i8('A')));

        // literal/variable with larger i16 literal: 4096 ? 3000
        c.writeOut(c.add(c.ge(literal::i16(4096), "y"), literal::i8('A')));

        // literal/literal constant folding, mixed widths: 42 ? 42
        c.writeOut(c.add(c.ge(literal::i8(42), literal::i16(42)), literal::i8('A')));
