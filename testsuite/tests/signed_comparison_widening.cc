...
c.function("main").begin(); {
  ...
  c.assign("x", literal::s16(-1));
  c.writeOut(c.add(c.lt("a", "x"), literal::i8('A'))); // A

  // s16(-1) < s8(127) -> true
  c.writeOut(c.add(c.lt("x", "a"), literal::i8('A'))); // B

  // s8(-1) > s16(-300) -> true
  c.assign("a", literal::s8(-1));
  c.assign("x", literal::s16(-300));
  c.writeOut(c.add(c.gt("a", "x"), literal::i8('A'))); // B

  // s16(-300) > s8(-1) -> false
  c.writeOut(c.add(c.gt("x", "a"), literal::i8('A'))); // A

  // equality boundaries through promoted comparison
  // s8(-1) <= s16(-1) -> true
  c.assign("a", literal::s8(-1));
  c.assign("x", literal::s16(-1));
  c.writeOut(c.add(c.le("a", "x"), literal::i8('A'))); // B

  // s8(-1) >= s16(-1) -> true
  c.writeOut(c.add(c.ge("a", "x"), literal::i8('A'))); // B

  // s8(-2) >= s16(-1) -> false
  c.assign("a", literal::s8(-2));
  c.assign("x", literal::s16(-1));
  c.writeOut(c.add(c.ge("a", "x"), literal::i8('A'))); // A

  // s16(-1) >= s8(-2) -> true
  c.writeOut(c.add(c.ge("x", "a"), literal::i8('A'))); // B
