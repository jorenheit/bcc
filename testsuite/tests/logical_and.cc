...
c.function("main").begin(); {
  ...
  c.write(c.add(c.land("a", "b"), literal::i8('A')));
  ...
  c.write(c.add(c.land("x", "y"), literal::i8('A')));
  ...
  c.write(c.add(c.land("a", "x"), literal::i8('A')));

  // 6. variable + literal: true AND false -> 0
  c.assign("a", literal::i8(0x66));
  c.write(c.add(c.land("a", literal::i8(0)), literal::i8('A')));

  // 7. literal + variable: true AND true -> 1
  c.assign("x", literal::i16(0x2345));
  c.write(c.add(c.land(literal::i16(0x7777), "x"), literal::i8('A')));

  // 8. literal + literal, constant-folded: true AND false -> 0
  c.write(c.add(c.land(literal::i16(0x9999), literal::i16(0)), literal::i8('A')));
