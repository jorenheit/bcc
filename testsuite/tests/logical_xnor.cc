...
c.beginFunction("main"); {
  ...
  c.beginBlock("entry"); {
    ...
    c.writeOut(c.add(c.lxnor("a", "b"), values::i8('A')));
    ...
    c.writeOut(c.add(c.lxnor("x", "y"), values::i8('A')));
    ...
    c.writeOut(c.add(c.lxnor("a", "x"), values::i8('A')));

    // 6. variable + literal: true XNOR false -> 0
    c.assign("a", values::i8(0x66));
    c.writeOut(c.add(c.lxnor("a", values::i8(0)), values::i8('A')));

    // 7. literal + variable: true XNOR true -> 1
    c.assign("x", values::i16(0x2345));
    c.writeOut(c.add(c.lxnor(values::i16(0x7777), "x"), values::i8('A')));

    // 8. literal + literal, constant-folded: true XNOR false -> 0
    c.writeOut(c.add(c.lxnor(values::i16(0x9999), values::i16(0)), values::i8('A')));

    // 9. i8 assign variant: true XNOR true -> 1
    c.assign("a", values::i8(0x42));
    c.lxnorAssign("a", values::i8(0x24));
