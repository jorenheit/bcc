...
TEST_BEGIN
    ...
    c.beginFunction("main"); {
      ...
      c.beginBlock("entry"); {
        ...
        c.writeOut(c.add(c.le("a", "b"), values::i8('A')));
        c.writeOut(c.add(c.le("b", "a"), values::i8('A')));
        ...
        c.writeOut(c.add(c.le("x", "y"), values::i8('A')));
        c.writeOut(c.add(c.le("y", "x"), values::i8('A')));

        // mixed i8/i16 variables: 20 ? 2000, then 2000 ? 20
        c.writeOut(c.add(c.le("a", "x"), values::i8('A')));
        c.writeOut(c.add(c.le("x", "a"), values::i8('A')));

        // variable/literal equality edge: 20 ? 20
        c.writeOut(c.add(c.le("a", values::i8(20)), values::i8('A')));

        // literal/variable with larger i16 literal: 4096 ? 3000
        c.writeOut(c.add(c.le(values::i16(4096), "y"), values::i8('A')));

        // literal/literal constant folding, mixed widths: 42 ? 42
        c.writeOut(c.add(c.le(values::i8(42), values::i16(42)), values::i8('A')));
