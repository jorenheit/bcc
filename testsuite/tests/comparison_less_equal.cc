...
c.function("main").begin(); {
  ...
  c.write(c.add(c.le("a", "b"), literal::i8('A')));
  c.write(c.add(c.le("b", "a"), literal::i8('A')));
  ...
  c.write(c.add(c.le("x", "y"), literal::i8('A')));
  c.write(c.add(c.le("y", "x"), literal::i8('A')));

  // mixed i8/i16 variables: 20 ? 2000, then 2000 ? 20
  c.write(c.add(c.le("a", "x"), literal::i8('A')));
  c.write(c.add(c.le("x", "a"), literal::i8('A')));

  // variable/literal equality edge: 20 ? 20
  c.write(c.add(c.le("a", literal::i8(20)), literal::i8('A')));

  // literal/variable with larger i16 literal: 4096 ? 3000
  c.write(c.add(c.le(literal::i16(4096), "y"), literal::i8('A')));

  // literal/literal constant folding, mixed widths: 42 ? 42
  c.write(c.add(c.le(literal::i8(42), literal::i16(42)), literal::i8('A')));
