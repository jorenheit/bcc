...
c.function("main").begin(); {
  ...
  c.write(c.add(c.le("a", "b"), literal::u8('A')));
  c.write(c.add(c.le("b", "a"), literal::u8('A')));
  ...
  c.write(c.add(c.le("x", "y"), literal::u8('A')));
  c.write(c.add(c.le("y", "x"), literal::u8('A')));

  // mixed u8/u16 variables: 20 ? 2000, then 2000 ? 20
  c.write(c.add(c.le("a", "x"), literal::u8('A')));
  c.write(c.add(c.le("x", "a"), literal::u8('A')));

  // variable/literal equality edge: 20 ? 20
  c.write(c.add(c.le("a", literal::u8(20)), literal::u8('A')));

  // literal/variable with larger u16 literal: 4096 ? 3000
  c.write(c.add(c.le(literal::u16(4096), "y"), literal::u8('A')));

  // literal/literal constant folding, mixed widths: 42 ? 42
  c.write(c.add(c.le(literal::u8(42), literal::u16(42)), literal::u8('A')));
